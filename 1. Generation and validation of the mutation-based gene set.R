...
cox <- coxph(Surv(futime, fustat) ~ risk, data=rt, ties="breslow") #Ö¸¶¨ties="breslow"
hr <- summary(cox)[["coefficients"]][, "Pr(>|z|)"]
summary(cox)
...
# Comparison of C-indexes among the mutation-based gene set, frameshift insertion/deletion (indel) mutation burden, tobacco mutation signature, UV signature, APOBEC signature, and DNA damage response pathway mutation. 
...
infile <- "2. frameshift insertiondeletion (indel) mutation burden/newdf-ÑµÁ·¼¯.csv"
clindf <- read.csv(infile, header=TRUE, check.names=FALSE)
dim(clindf)
clindf[1:6,]
colnames(clindf)
...
# Comparison of C-indexes among the mutation-based gene set, B2M mutation, JAK1 mutation, JAK2 mutation, KRAS mutation, TP53 mutation, PTEN mutation, STK11 mutation, and BAP1 mutation
